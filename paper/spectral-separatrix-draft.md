...
In our symmetric model, the coexistence saddle exists only in a narrow window ($\Delta J_\times \approx 0.01$). However, biological circuits have heterogeneous connectivity, non-uniform firing thresholds, and spatially structured inhibition (Kilpatrick et al., 2013).

Heterogeneity has two key effects. First, it breaks the continuous rotational symmetry, converting the Goldstone modes into "soft modes" with small but nonzero eigenvalues (Kilpatrick et al., 2013; Poll et al., 2015). Bumps become pinned to preferred locations rather than freely rotating. Second, heterogeneity creates a distribution of effective local $J_\times$ values across the network. Some micro-circuits hit criticality before others, smearing the sharp transition into a broader regime where saddle-like dynamics persist.

[Section to be completed with heterogeneity experiment results.]
