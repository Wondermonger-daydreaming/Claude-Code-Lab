"""
Generate Figure 5 (v6): Phase Diagram in (J_cross, drive_strength) Space
=========================================================================
Publication-quality phase diagram for the spectral separatrix paper.

v6 additions over the original (generate_fig8_phase_diagram.py):
  - Shaded ±1 SE error bands on key contour lines (5%, 20%, 50%)
  - Three vertical dashed lines for deterministic thresholds:
      J_cross^onset ≈ 0.25  (Kramers onset)
      J_cross*      ≈ 0.3485 (pitchfork bifurcation)
      J_cross^exist ≈ 0.358  (existence boundary)
  - Text labels on vertical lines
  - Output to paper/figures/fig5_phase_diagram.png

Data: 128,000-trial parameter sweep from 4 parallel agents (500 trials/point).

Usage:
    python3 generate_fig5_phase_diagram_v6.py
"""
...
OUTPUT_PATH = "/home/gauss/Claude-Code-Lab/paper/figures/fig5_phase_diagram.png"

# ── Deterministic thresholds ───────────────────────────────────────
J_KRAMERS_ONSET = 0.25
J_PITCHFORK = 0.3485
J_EXIST = 0.358

# ── Number of trials per grid point (for SE calculation) ──────────
N_TRIALS = 500
...
def compute_se_grid(swap_grid, n_trials=N_TRIALS):
    """
    Compute binomial standard error for each point on the interpolated grid.
    SE = sqrt(p * (1 - p) / n), where p = swap_rate / 100.
    Returns SE in percentage points (same scale as swap_grid).
    """
    p = np.clip(swap_grid / 100.0, 0.001, 0.999)  # avoid exact 0/1
    se = np.sqrt(p * (1 - p) / n_trials) * 100.0   # back to percentage
    return se


def generate_figure():
    """Generate the publication-quality phase diagram with error bands."""
    ...
    # ── Compute SE grid ───────────────────────────────────────────
    se_grid = compute_se_grid(swap_grid)
    print(f"\n  SE range: [{se_grid.min():.2f}%, {se_grid.max():.2f}%]")

    # Grids offset by ±1 SE for error band contours
    swap_grid_plus = swap_grid + se_grid
    swap_grid_minus = np.clip(swap_grid - se_grid, 0, None)
    ...
    # ── SE error bands on key contours (5%, 20%, 50%) ─────────────
    # Strategy: draw contours of swap_grid ± SE at each target level.
    # This produces paired dotted lines flanking the main contour,
    # showing where the contour could lie given binomial uncertainty.
    # Also fill between with a subtle semi-transparent wash.
    se_band_levels = [5, 20, 50]

    # Filled band: contourf between (level - mean_SE) and (level + mean_SE)
    mean_se = se_grid.mean()
    for level in se_band_levels:
        lo = max(0, level - mean_se)
        hi = level + mean_se
        try:
            ax.contourf(
                J_mesh, D_mesh, swap_grid,
    ...
    # Dotted lines: contours of (swap ± SE) at each level
    for level in se_band_levels:
        for shifted_grid in [swap_grid_plus, swap_grid_minus]:
            try:
                ax.contour(
                    J_mesh, D_mesh, shifted_grid,
                    levels=[level],
                    colors='white', linewidths=0.5, linestyles='dotted', alpha=0.4,
                )
            except Exception:
                pass
    ...
    # ── Critical vertical lines (deterministic thresholds) ────────
    # Kramers onset
    ax.axvline(x=J_KRAMERS_ONSET, color='#66bb6a', ls='--', lw=1.4,
               alpha=0.9, zorder=10)
    # Pitchfork bifurcation
    ax.axvline(x=J_PITCHFORK, color='#ff9100', ls='--', lw=1.5,
               alpha=0.9, zorder=10)
    # Existence boundary
    ax.axvline(x=J_EXIST, color='#ce93d8', ls=':', lw=1.5,
               alpha=0.9, zorder=10)
    ...
    # ── Text labels on vertical lines ─────────────────────────────
    # Stagger vertically to avoid overlap (log scale makes 0.25 and 0.35 close)
    # Kramers onset label (left side, high up)
    ax.text(J_KRAMERS_ONSET * 0.88, 7.8, 'Kramers\nonset',
            fontsize=6.5, color='#66bb6a', ha='right', va='top',
            fontweight='bold', path_effects=stroke_dk, zorder=20)
    # Pitchfork label (between pitchfork and existence, staggered lower)
    ax.text(J_PITCHFORK, 5.45, 'pitchfork',
            fontsize=6.5, color='#ff9100', ha='right', va='center',
            fontweight='bold', rotation=90,
            path_effects=stroke_dk, zorder=20)
    # Existence boundary label (right side of line, placed low to avoid WTA onset)
    ax.text(J_EXIST * 1.05, 2.7, 'existence\nboundary',
            fontsize=6.5, color='#ce93d8', ha='left', va='center',
            fontweight='bold',
            path_effects=stroke_dk, zorder=20)
    ...
    # ── Legend ─────────────────────────────────────────────────────
    legend_elements = [
        Line2D([0], [0], color='#66bb6a', ls='--', lw=1.4,
               label=r'$J_{\times}^{\,\mathrm{onset}} \approx 0.25$  (Kramers onset)'),
        Line2D([0], [0], color='#ff9100', ls='--', lw=1.5,
               label=r'$J_{\times}^{\,*} \approx 0.349$  (pitchfork)'),
        Line2D([0], [0], color='#ce93d8', ls=':', lw=1.5,
               label=r'$J_{\times}^{\,\mathrm{exist}} \approx 0.358$  (existence limit)'),
        Line2D([0], [0], color='white', ls='dotted', lw=0.7, alpha=0.6,
               label=r'$\pm 1\,\mathrm{SE}$ contour bands'),
    ]
    legend = ax.legend(
    ...
    # ── Delta-J bracket ───────────────────────────────────────────
    # Visible because log scale gives room between 0.3485 and 0.358
    y_bkt = 1.2
    ax.plot([J_PITCHFORK, J_PITCHFORK], [y_bkt - 0.15, y_bkt + 0.15],
            color='#ffeb3b', lw=1.3, zorder=25)
    ax.plot([J_EXIST, J_EXIST], [y_bkt - 0.15, y_bkt + 0.15],
            color='#ffeb3b', lw=1.3, zorder=25)
    ax.plot([J_PITCHFORK, J_EXIST], [y_bkt, y_bkt],
            color='#ffeb3b', lw=1.3, zorder=25)
    ax.text(
...
def print_data_summary(j_vals, d_vals, swap_vals):
    ...
    print("\n" + "=" * 60)
    print("DATA SUMMARY FOR FIGURE 5 (v6)")
    print("=" * 60)
    ...
    # SE at key values
    print("\n  Binomial SE at key swap rates:")
    for p_pct in [5, 10, 20, 30, 50]:
        p = p_pct / 100.0
        se = np.sqrt(p * (1 - p) / N_TRIALS) * 100
        print(f"    p = {p_pct:2d}%: SE = {se:.2f}%")
